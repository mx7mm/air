...
- `AIR_AUTO_UPDATE=0` (manual update trigger)
...
If `/data/updates/current/VERSION` exists, runtime uses it as effective version source.
Otherwise it falls back to `AIR_VERSION_FILE` from config (default `/etc/air/VERSION`).
